...
function displayDestinations(items) {
    ...
    grid.innerHTML = items.map(item => `
        <div class="bg-white rounded-2xl shadow-sm overflow-hidden hover:shadow-xl transition-all duration-300 group border border-slate-100 relative text-slate-800">
            <!-- Nút thêm vào tour (Trip Planner) -->
            ${!item.isCultureTopic ? `
                    ...
                    <div class="absolute bottom-3 left-3 flex flex-wrap gap-2 z-20">
                        ${(item.cultureKeywords || []).map(kw => `
                            <span onclick="event.stopPropagation(); openModal(${kw.linkId})" 
                                  class="bg-emerald-600/90 backdrop-blur-md text-white text-[9px] px-2 py-1 rounded font-bold hover:bg-orange-500 transition shadow-lg uppercase">
                                # ${kw.label}
                            </span>
                        `).join('')}
                    </div>
...
function interpretWeather(code) {
    const map = { 
        0: { icon: "fa-sun text-yellow-400", txt: "Trời nắng" }, 
        ...
        61: { icon: "fa-cloud-rain text-blue-400", txt: "Có mưa" } 
    };
    return map[code] || { icon: "fa-cloud", txt: "Ổn định" };
}

// ==========================================
// 4. HÀM MỞ MODAL (DÀNH CHO ĐỊA DANH & VĂN HÓA)
// ==========================================
...
async function openModal(id) {
    ...
    if (item.isCultureTopic) {
        ...
        content.innerHTML = `
            ...
            <div class="p-8 md:p-16 space-y-10 bg-white text-slate-800">
                ...
                <div class="text-center pt-6"><button onclick="closeModal()" class="bg-emerald-900 text-white px-10 py-3 rounded-full font-bold uppercase hover:bg-black transition">Quay lại</button></div>
            </div>`;
    } else {
        // --- Giao diện Địa danh ---
        const weather = await fetchWeather(item.lat || 23.2, item.lng || 105.2);
        const curr = interpretWeather(weather?.current_weather?.weathercode || 0);
        const linkedCultures = (item.relatedCultureIds || []).map(cId => destinationsData.find(d => d.id === cId)).filter(Boolean);

        content.innerHTML = `
            <div class="relative h-72 md:h-96"><img src="${item.image}" class="w-full h-full object-cover"><div class="absolute inset-0 bg-gradient-to-t from-black/80 flex items-end p-8 text-white uppercase text-4xl md:text-6xl font-bold">${item.name}</div></div>
            <div class="p-6 md:p-10 grid grid-cols-1 lg:grid-cols-3 gap-10 bg-white text-slate-900 text-sm">
                <div class="lg:col-span-2 space-y-12 text-left">
                    <section><h3 class="text-2xl font-bold border-l-8 border-orange-500 pl-4 mb-4 text-emerald-900 uppercase">Tổng quan</h3><p class="text-gray-700 italic leading-relaxed text-lg">"${item.experience || item.desc}"</p></section>
                    
                    <!-- Khối thời tiết -->
                    <section class="bg-slate-900 text-white p-8 rounded-[40px] shadow-2xl flex flex-col md:flex-row items-center justify-between gap-6 border border-white/5">
                        <div class="flex items-center gap-4 border-b md:border-b-0 md:border-r border-white/10 pb-4 md:pb-0 md:pr-10 text-left">
                            <i class="fas ${curr.icon} text-4xl"></i>
                            <div><p class="text-3xl font-bold">${weather ? weather.current_weather.temperature : '--'}°C</p><p class="text-xs uppercase text-emerald-400 font-bold">${curr.txt}</p></div>
                        </div>
                        <div class="flex-1 grid grid-cols-3 gap-3 w-full">
                            ${weather ? weather.daily.time.slice(1,4).map((t, i) => `<div class="bg-white/5 p-2 rounded-2xl text-center"><p class="text-[10px] opacity-40 uppercase">${new Date(t).toLocaleDateString('vi-VN', {weekday: 'short'})}</p><i class="fas ${interpretWeather(weather.daily.weathercode[i+1]).icon} text-sm my-1 text-emerald-400"></i><p class="font-bold text-[10px]">${Math.round(weather.daily.temperature_2m_max[i+1])}°</p></div>`).join('') : ''}
                        </div>
                    </section>

                    <!-- Bản sắc văn hóa (Danh sách dọc) -->
                    <section class="bg-stone-900 text-white p-8 md:p-10 rounded-[40px] shadow-2xl border border-white/10">
                        <h3 class="text-xl font-bold text-orange-400 mb-8 uppercase border-b border-white/10 pb-2 italic font-bold">Bản sắc văn hóa</h3>
                        <div class="space-y-6 mb-10 text-gray-300">
                            <div class="border-l-2 border-emerald-500/30 pl-4"><p class="text-emerald-400 font-bold uppercase text-[10px] mb-1">Lễ hội:</p><p class="text-sm leading-relaxed">${item.culture?.festival || 'Cập nhật sau'}</p></div>
                            <div class="border-l-2 border-emerald-500/30 pl-4"><p class="text-emerald-400 font-bold uppercase text-[10px] mb-1">Trang phục:</p><p class="text-sm leading-relaxed">${item.culture?.costume || 'Cập nhật sau'}</p></div>
                            <div class="border-l-2 border-emerald-500/30 pl-4"><p class="text-emerald-400 font-bold uppercase text-[10px] mb-1">Phong tục:</p><p class="text-sm leading-relaxed">${item.culture?.customs || 'Cập nhật sau'}</p></div>
                        </div>
                        <div class="mt-8 pt-8 border-t border-white/5 text-center bg-white/5 -mx-10 -mb-10 p-10 rounded-b-[40px]">
                            <p class="text-[10px] font-bold text-gray-400 uppercase mb-4 tracking-widest">Khám phá chi tiết chuyên đề:</p>
                            <div class="flex flex-wrap justify-center gap-3">${linkedCultures.map(c => `<button onclick="openModal(${c.id})" class="bg-emerald-600 text-white px-5 py-2 rounded-xl font-bold text-[10px] hover:bg-orange-500 transition shadow-lg transform hover:scale-105 italic font-bold uppercase"># ${c.name}</button>`).join('')}</div>
                        </div>
                    </section>

                    <section><h3 class="text-2xl font-bold border-l-8 border-orange-500 pl-4 mb-6 text-emerald-900 uppercase font-bold">Ẩm thực</h3><div class="flex flex-col md:flex-row gap-8 bg-white p-6 rounded-3xl shadow-sm border border-slate-100 text-slate-800 text-left items-center"><img src="${item.food?.image || 'images/default-food.jpg'}" class="w-full md:w-48 h-32 object-cover rounded-2xl shadow-md"><div class="flex-1 flex flex-col justify-center text-sm"><h4 class="text-xl font-bold text-emerald-900 mb-1 uppercase">${item.food?.name || 'Đặc sản địa phương'}</h4><p class="text-orange-600 font-bold text-lg mb-2 underline italic">${item.food?.price || ''}</p><p class="text-gray-500 italic"><i class="fas fa-map-marker-alt mr-2 font-bold"></i><b>Tại:</b> ${item.food?.location || 'Chợ phiên'}</p></div></div></section>
                ...
                <!-- Cột phải (Sidebar) -->
                <div class="lg:col-span-1 space-y-8 text-slate-800">
                    <div class="bg-emerald-900 text-white p-8 rounded-[40px] shadow-xl"><h3 class="text-xl font-bold mb-4 text-orange-400 uppercase tracking-tighter font-bold">Lịch trình</h3><p class="leading-relaxed opacity-80 text-sm font-light italic">${item.logistics?.itinerary2D || 'Liên hệ để biết thêm'}</p></div>
...
async function sendMessage() {
    ...
    try {
        const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
            ...
            body: JSON.stringify({ model: "llama-3.1-8b-instant", messages: [{ role: "system", content: "Bạn là trợ lý Hà Giang chuyên nghiệp. Trả lời ngắn gọn bằng tiếng Việt." }, { role: "user", content: userMsg }] })
        });
...
function addToTrip(id) { 
    if (!selectedTripIds.includes(id)) { 
        ...
        alert("Địa điểm này đã có trong hành trình!");
    }
}
...
async function generateAITrip() {
    ...
    content.scrollTo(0, content.scrollHeight);
    // Gọi AI để lập tour thực sự...
}

// ==========================================
// 7. KHỞI CHẠY (CHỈ CÓ 1 LẦN DUY NHẤT)
// ==========================================
window.onload = () => {
    if(typeof displayDestinations === 'function') {
