...
async function openModal(id) {
    ...
    if (item.isCultureTopic) {
        content.innerHTML = `
            <div class="relative h-72 md:h-[400px]"><img src="${item.image}" class="w-full h-full object-cover"><div class="absolute inset-0 bg-black/60 flex items-center justify-center p-6 text-white uppercase text-4xl md:text-6xl font-bold">${item.name}</div></div>
            <div class="p-8 md:p-16 space-y-12 bg-white text-slate-800 text-left">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ${(item.sections || []).map(sec => `<div class="bg-stone-50 p-8 rounded-3xl border-l-8 border-emerald-700 shadow-sm"><h4 class="font-bold text-2xl text-emerald-900 mb-4 uppercase">${sec.title}</h4><p class="text-gray-600 leading-relaxed text-lg">${sec.content}</p></div>`).join('')}
                </div>
        ...
        content.innerHTML = `
            ...
            <div class="p-6 md:p-10 grid grid-cols-1 lg:grid-cols-3 gap-10 bg-white text-slate-900 text-sm">
                <div class="lg:col-span-2 space-y-10 text-left">
                    ...
                    <section class="bg-slate-900 text-white p-8 rounded-[40px] shadow-2xl flex flex-col md:flex-row items-center justify-between gap-6">
                        <div class="flex items-center gap-4 border-b md:border-b-0 md:border-r border-white/10 pb-4 md:pb-0 md:pr-10">
                            ...
                            <div><p class="text-3xl font-bold">${res?.current_weather?.temperature || '--'}°C</p><p class="text-[10px] uppercase tracking-widest text-emerald-400 font-bold">${curr.txt}</p></div>
                        </div>
                        <div class="flex-1 grid grid-cols-3 gap-2 w-full">
                            ${res ? res.daily.time.slice(1,4).map((t, i) => `<div class="bg-white/5 p-2 rounded-2xl text-center"><p class="text-[10px] opacity-40">${new Date(t).toLocaleDateString('vi-VN', {weekday: 'short'})}</p><i class="fas ${interpretWeather(res.daily.weathercode[i+1]).icon} text-[10px] my-1 text-emerald-400"></i><p class="font-bold text-[10px]">${Math.round(res.daily.temperature_2m_max[i+1])}°</p></div>`).join('') : ''}
                        </div>
                    </section>
                    <section class="bg-stone-900 text-white p-8 rounded-[40px] border border-white/10 shadow-2xl">
                        <h3 class="text-xl font-bold text-orange-400 mb-8 border-b border-white/10 pb-2 uppercase italic tracking-widest italic font-bold">Bản sắc văn hóa</h3>
                        ...
                        <div class="mt-8 pt-8 border-t border-white/5 text-center bg-white/5 -mx-8 -mb-8 p-8 rounded-b-[40px]">
                            <div class="flex flex-wrap justify-center gap-3">${linked.map(c => `<button onclick="openModal(${c.id})" class="bg-emerald-600 text-white px-5 py-2 rounded-full font-bold text-[10px] hover:bg-orange-500 transition shadow-lg tracking-widest italic font-bold uppercase"># ${c.name}</button>`).join('')}</div>
                        </div>
                    </section>
                    <section><h3 class="text-2xl font-bold uppercase text-emerald-900">Bản đồ</h3><iframe src="${item.locationMap}" class="w-full h-80 rounded-3xl border-0 shadow-lg" loading="lazy"></iframe></section>
                </div>
                <div class="lg:col-span-1 space-y-8">
                    <div class="bg-emerald-900 text-white p-8 rounded-[40px] shadow-xl"><h3 class="text-xl font-bold mb-4 text-orange-400 uppercase tracking-tighter">Lịch trình</h3><p class="opacity-80">${item.logistics?.itinerary2D || 'Liên hệ'}</p></div>
                    <div class="bg-orange-50 p-6 rounded-[30px] border border-orange-100 text-center"><p class="text-[10px] text-orange-600 font-bold uppercase mb-1">Chi phí dự kiến</p><p class="text-xl font-bold text-orange-700 italic">${item.logistics?.estimatedCost || 'Tùy nhu cầu'}</p></div>
                </div>
            </div>`;
    }
...
async function sendMessage() {
    ...
    content.innerHTML += `<div class="bg-blue-600 text-white p-3 rounded-2xl ml-auto max-w-[85%] text-xs mb-2 shadow-sm">${msg}</div>`;
    input.value = "";
