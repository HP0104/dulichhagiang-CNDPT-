    ...
    if (item.isCultureTopic) {
        content.innerHTML = `
            ...
            <div class="p-8 md:p-16 space-y-10 bg-white text-slate-800 text-left overflow-y-auto">
                ...
                <div class="text-center pt-10"><button onclick="closeModal()" class="bg-emerald-900 text-white px-10 py-3 rounded-full font-bold uppercase hover:bg-black transition">Đóng lại</button></div>
            </div>`;
    } else {
        let weatherHTML = "";
        try {
        ...
        content.innerHTML = `
            ...
            <div class="p-6 md:p-10 grid grid-cols-1 lg:grid-cols-3 gap-10 bg-white text-slate-900 text-sm overflow-y-auto">
                <div class="lg:col-span-2 space-y-10 text-left">
                    ...
                    <section class="bg-stone-900 text-white p-8 md:p-10 rounded-[40px] shadow-2xl border border-white/10">
                        ...
                        <div class="space-y-6 mb-10 text-gray-300">
                            <div class="border-l-2 border-emerald-500/30 pl-4"><p class="text-emerald-400 font-bold uppercase text-[10px] mb-1">Lễ hội:</p><p class="text-gray-300 text-sm leading-relaxed">${item.culture?.festival || 'Đang cập nhật'}</p></div>
                            <div class="border-l-2 border-emerald-500/30 pl-4"><p class="text-emerald-400 font-bold uppercase text-[10px] mb-1">Trang phục:</p><p class="text-gray-300 text-sm leading-relaxed">${item.culture?.costume || 'Đang cập nhật'}</p></div>
                            <div class="border-l-2 border-emerald-500/30 pl-4"><p class="text-emerald-400 font-bold uppercase text-[10px] mb-1">Phong tục:</p><p class="text-gray-300 text-sm leading-relaxed">${item.culture?.customs || 'Đang cập nhật'}</p></div>
                        </div>
                        <div class="mt-8 pt-8 border-t border-white/5 text-center bg-white/5 -mx-10 -mb-10 p-10 rounded-b-[40px]">
                            <div class="flex flex-wrap justify-center gap-3">${linkedCultures.map(c => `<button onclick="openModal(${c.id})" class="bg-emerald-600 hover:bg-orange-500 text-white px-5 py-2 rounded-xl text-[10px] font-bold transition shadow-lg transform hover:scale-105 italic"># ${c.name}</button>`).join('')}</div>
                        </div>
                    </section>
                    <section><h3 class="text-2xl font-bold border-l-8 border-emerald-500 pl-4 mb-6 text-emerald-900 uppercase">Ẩm thực vùng cao</h3><div class="flex flex-col md:flex-row gap-8 bg-white p-6 rounded-3xl shadow-sm border border-slate-100 text-slate-800 text-left items-center"><img src="${item.food?.image || 'images/default-food.jpg'}" class="w-full md:w-52 h-40 object-cover rounded-2xl shadow-md"><div class="flex-1 flex flex-col justify-center"><h4 class="text-xl font-bold text-emerald-900 mb-2 uppercase">${item.food?.name || 'Đặc sản địa phương'}</h4><p class="text-orange-600 font-bold mb-1">${item.food?.price || ''}</p><p class="text-gray-500 italic text-xs">Tại: ${item.food?.location || 'Chợ phiên'}</p></div></div></section>
                ...
                <div class="lg:col-span-1 space-y-8">
                    <div class="bg-emerald-900 text-white p-8 rounded-[40px] shadow-xl"><h3 class="text-xl font-bold mb-6 border-b border-emerald-700 pb-2 text-orange-400 uppercase text-sm font-bold">Lịch trình</h3><div class="space-y-6 text-sm opacity-90 leading-loose italic"><div><p class="font-bold text-emerald-300 uppercase text-[10px]">Tour 2 Ngày:</p><p>${item.logistics?.itinerary2D || 'Cập nhật sau'}</p></div><div><p class="font-bold text-emerald-300 uppercase text-[10px]">Tour 3 Ngày:</p><p>${item.logistics?.itinerary3D || 'Cập nhật sau'}</p></div></div></div>
                    <div class="bg-white p-8 rounded-[40px] border border-slate-100 shadow-md text-center text-slate-800"><p class="text-[10px] text-orange-600 font-bold uppercase mb-1 underline font-bold">Chi phí dự kiến</p><p class="text-xl font-bold text-orange-700 italic font-bold">${item.logistics?.estimatedCost || 'Tùy nhu cầu'}</p></div>
                </div>
            </div>`;
    }
...
function updateTripUI() {
    ...
    if (selectedTripIds.length > 0) {
        ...
        document.getElementById('selected-list').innerHTML = selectedTripIds.map(id => {
            const item = destinationsData.find(d => d.id === id);
...
async function generateAITrip() {
    ...
    const content = document.getElementById('chat-content');
    content.innerHTML += `<div class="bg-orange-100 p-4 rounded-2xl text-[10px] italic text-slate-800 shadow-md mb-2 border border-orange-200">AI đang lập tour đi: ${names}...</div>`;
    content.scrollTo(0, content.scrollHeight);
    ...
    try {
        const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
            method: "POST",
            headers: { "Content-Type": "application/json", "Authorization": `Bearer ${API_KEY}` },
            body: JSON.stringify({ model: "llama-3.1-8b-instant", messages: [{ role: "system", content: "Bạn là tour guide Hà Giang. Lập lịch trình 2 ngày và tính chi phí cực ngắn gọn cho danh sách điểm khách chọn." }, { role: "user", content: "Lập tour: " + names }] })
        });
        const data = await res.json();
        content.innerHTML += `<div class="bg-emerald-50 p-4 rounded-2xl self-start max-w-[95%] text-xs border border-emerald-200 shadow-lg mb-2">${data.choices[0].message.content.replace(/\n/g, '<br>')}</div>`;
