...
async function openModal(id) {
    ...
    if (item.isCultureTopic) {
        ...
        let weatherHTML = "";
        try {
            const lat = item.lat || 23.2; const lng = item.lng || 105.2;
            ...
            const curr = interpretWeather(res.current_weather.weathercode);
            weatherHTML = `
            ...
            <div class="p-6 md:p-10 grid grid-cols-1 lg:grid-cols-3 gap-10 bg-white text-slate-900 text-sm overflow-y-auto">
                <div class="lg:col-span-2 space-y-10 text-left">
                    ...
                    <section class="bg-stone-900 text-white p-8 md:p-10 rounded-[40px] shadow-2xl border border-white/10">
                        <h3 class="text-xl font-bold text-orange-400 mb-8 border-b border-white/10 pb-4 uppercase italic tracking-widest italic font-bold">Bản sắc văn hóa</h3>
                        <div class="space-y-6 mb-12 text-gray-300">
                            <div class="border-l-2 border-emerald-500/30 pl-4"><p class="text-emerald-400 font-bold uppercase text-[10px] mb-1">Lễ hội:</p><p class="text-gray-300 text-sm leading-relaxed">${item.culture?.festival || 'Cập nhật'}</p></div>
                            <div class="border-l-2 border-emerald-500/30 pl-4"><p class="text-emerald-400 font-bold uppercase text-[10px] mb-1">Trang phục:</p><p class="text-gray-300 text-sm leading-relaxed">${item.culture?.costume || 'Cập nhật'}</p></div>
                            <div class="border-l-2 border-emerald-500/30 pl-4"><p class="text-emerald-400 font-bold uppercase text-[10px] mb-1">Phong tục:</p><p class="text-gray-300 text-sm leading-relaxed">${item.culture?.customs || 'Cập nhật'}</p></div>
                        </div>
                        <div class="mt-8 pt-8 border-t border-white/5 text-center bg-white/5 -mx-10 -mb-10 p-10 rounded-b-[40px]">
                            <div class="flex flex-wrap justify-center gap-3">${linked.map(c => `<button onclick="openModal(${c.id})" class="bg-emerald-600 text-white px-5 py-2 rounded-xl font-bold text-[10px] hover:bg-orange-500 transition shadow-lg transform hover:scale-105 italic font-bold uppercase"># ${c.name}</button>`).join('')}</div>
                        </div>
                    </section>
                ...
                <div class="lg:col-span-1 space-y-8">
                    ...
                    <div class="bg-orange-50 p-8 rounded-[40px] border border-orange-200 text-center text-slate-800"><p class="text-[10px] text-orange-600 font-bold uppercase mb-1 underline font-bold italic">Chi phí dự kiến</p><p class="text-xl font-bold text-orange-700 italic font-bold">${item.logistics?.estimatedCost || 'Tùy nhu cầu'}</p></div>
                </div>
            </div>`;
    }
...
async function sendMessage() {
    ...
    try {
        const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
            ...
            body: JSON.stringify({ model: "llama-3.1-8b-instant", messages: [{ role: "system", content: "Trợ lý Hà Giang. Trả lời cực ngắn bằng tiếng Việt." }, { role: "user", content: msg }] })
        });
        const data = await res.json();
        content.innerHTML += `<div class="bg-emerald-100 text-emerald-900 p-3 rounded-2xl self-start max-w-[85%] text-xs border border-emerald-200 shadow-sm mb-2">${data.choices[0].message.content}</div>`;
...
async function generateAITrip() {
    ...
    document.getElementById('chat-content').innerHTML += `<div class="bg-orange-100 p-3 rounded-2xl text-[10px] italic shadow-sm mb-2">AI đang lập tour đi: ${names}...</div>`;
    try {
        const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
            method: "POST", headers: { "Content-Type": "application/json", "Authorization": `Bearer ${API_KEY}` },
