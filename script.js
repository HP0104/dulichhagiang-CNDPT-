...
async function openModal(id) {
    ...
    if (item.isCultureTopic) {
        content.innerHTML = `<div class="relative h-72 md:h-[400px]"><img src="${item.image}" class="w-full h-full object-cover"><div class="absolute inset-0 bg-black/60 flex items-center justify-center p-6 text-white uppercase text-4xl md:text-6xl font-bold text-center">${item.name}</div></div><div class="p-8 md:p-16 space-y-10 bg-white text-slate-800 text-left"><div class="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">${(item.sections || []).map(sec => `<div class="bg-stone-50 p-8 rounded-3xl border-l-8 border-emerald-700 shadow-sm"><h4 class="font-bold text-xl text-emerald-900 mb-2 uppercase">${sec.title}</h4><p class="text-gray-600 text-sm leading-relaxed">${sec.content}</p></div>`).join('')}</div><div class="text-center pt-6"><button onclick="closeModal()" class="bg-emerald-900 text-white px-10 py-3 rounded-full font-bold uppercase hover:bg-black transition shadow-lg">Đóng</button></div></div>`;
    } else {
        const linked = (item.relatedCultureIds || []).map(cId => destinationsData.find(d => d.id === cId)).filter(Boolean);
        content.innerHTML = `<div class="relative h-72 md:h-96"><img src="${item.image}" class="w-full h-full object-cover"><div class="absolute inset-0 bg-gradient-to-t from-black/90 flex items-end p-8 text-white uppercase text-4xl md:text-6xl font-bold">${item.name}</div></div><div class="p-6 md:p-10 grid grid-cols-1 lg:grid-cols-3 gap-10 bg-white text-slate-900 text-sm text-left"><div class="lg:col-span-2 space-y-10"><section><h3 class="text-2xl font-bold border-l-8 border-orange-500 pl-4 mb-4 text-emerald-900 uppercase">Tổng quan</h3><p class="text-gray-700 italic">"${item.experience || item.desc}"</p></section><section class="bg-stone-900 text-white p-8 md:p-10 rounded-[40px] shadow-2xl"><h3 class="text-xl font-bold text-orange-400 mb-8 border-b border-white/10 pb-2 uppercase">Bản sắc văn hóa</h3><div class="space-y-6 mb-10 text-gray-300"><div><p class="text-emerald-400 font-bold uppercase text-[10px]">Lễ hội:</p><p>${item.culture?.festival || 'Cập nhật'}</p></div><div><p class="text-emerald-400 font-bold uppercase text-[10px]">Trang phục:</p><p>${item.culture?.costume || 'Cập nhật'}</p></div></div><div class="pt-6 border-t border-white/5 flex flex-wrap gap-2">${linked.map(c => `<button onclick="openModal(${c.id})" class="bg-emerald-600 text-white px-5 py-2 rounded-xl font-bold text-[10px] hover:bg-orange-500 transition shadow-lg uppercase"># ${c.name}</button>`).join('')}</div></section><section><h3 class="text-2xl font-bold mb-6 text-emerald-900 uppercase">Bản đồ</h3><iframe src="${item.locationMap}" class="w-full h-80 rounded-3xl border-0 shadow-lg" loading="lazy"></iframe></section></div><div class="lg:col-span-1 space-y-8 text-slate-800"><div class="bg-emerald-900 text-white p-8 rounded-[40px] shadow-xl"><h3 class="text-xl font-bold mb-4 text-orange-400 uppercase">Lịch trình</h3><p class="opacity-80 leading-relaxed italic text-sm">${item.logistics?.itinerary2D || 'Liên hệ'}</p></div><div class="bg-orange-50 p-8 rounded-[40px] border border-orange-100 text-center"><p class="text-[10px] text-orange-600 font-bold uppercase mb-1">Chi phí dự kiến</p><p class="text-xl font-bold text-orange-700 italic">${item.logistics?.estimatedCost || 'Tùy nhu cầu'}</p></div></div></div>`;
    }
...
async function sendMessage() {
    ...
    try {
        const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
            ...
            body: JSON.stringify({ model: "llama-3.1-8b-instant", messages: [{ role: "system", content: "Trợ lý Hà Giang chuyên nghiệp. Trả lời cực ngắn bằng tiếng Việt." }, { role: "user", content: msg }] })
        });
